...
| AbilityGui (client) | none | The ability bar, bottom centre: one button per owned ability, the shared charge under them, and the key. Bottom centre rather than a sixth chip down the right edge, because a selector is the one thing on screen meant to be pressed rather than read, and because the corners are the thumbstick and the jump button on a phone. Ownership, tier and selection are read straight off the replicated player attributes and never off the remote, so a client that joined late or reset its GUI is correct on its first frame; the remote supplies the charge and the events. Hidden entirely at zero owned, which is how a player who has not been to the stall is never told about a key they do not have. Hold `Q` to use, `1`-`9` or a tap to select, D-pad to cycle. |
| SprintService | none | Sprint, and the one ability nothing sells: every player has it from the first floor, which is why the meter regenerates on a clock instead of refilling at a `LevelTrigger` the way a bought per-floor budget does. No tag, no tier, no profile field, and no capacity of zero to hide the HUD chip behind. Drains only while `Humanoid.MoveDirection` is non-zero and regenerates only while not sprinting, so the meter is never doing both: holding the key still is neither a cost nor a free refill, and a ride under PlatformStand spends nothing. A sprint that empties cannot resume under a key still held, because the client only sends `start` on a fresh press. Speed is a `WalkSpeedResolver` factor named `Sprint`, never a write to the humanoid. |
