  ...
  src/
    ...
    server/                 -> ServerScriptService
      ...
      Enemy/                  ModuleScripts, the enemy AI (see the enemy section below)
        ...
        EnemySpawner.lua  EnemyAlert.lua  EnemyWard.lua
...
| PickupService | `Coin`, `Powerup` | Server owns the count in `leaderstats.Coins`; a taken pickup is hidden with `Transparency`/`CanTouch` and comes back on a timer, never destroyed. Collected two ways: `Touched` for the zero-latency path, and a `PickupRadius` sweep per living player because a 3.4-stud disc reached by touch alone had to be walked into almost exactly. The sweep is one `GetPartBoundsInRadius` per player answered by the engine broadphase, not a distance test against the 1,068 collectibles per section; both paths funnel into the same handler and guard on the same taken table, neither yielding between reading that guard and setting it. The kind is rolled here, on touch, from `PowerupRoll`, not read off the orb; runtime randomness, deliberately not seeded off the world seed. Effects are restore closures, one active per player. Ghost sets `Unseen` on the character and Freeze sets `EnemyFreezeUntil` on `workspace`, both read by EnemyService; `Unseen` is this service's alone, the Cloak ability having its own `Cloaked` flag beside it so that neither can clear the other's out from under it; Reveal is drawn entirely by the client and has no server effect at all; CoinBoost pays a lump sum that is never taken back, so it is not in the undo list, plus a `coinBonus` multiplier that is. Speed is a `WalkSpeedResolver` factor named `Powerup`, so its undo removes a term rather than restoring a remembered number. Fires the `PickupUpdate` RemoteEvent, which carries only events (the ding, which powerup started); the number itself rides replicated leaderstats, so a late client reads it correctly. |
| SaveService | `ShopItem`, `TowerStart` | The shop and the furthest-section credit; the profile it reads lives in `PlayerProfiles`. Purchases go through each pedestal's generated ProximityPrompt, deduct from `leaderstats.Coins`, and report bought/poor/maxed over the `ShopUpdate` RemoteEvent. The stall sells passives and abilities out of one `Config.Shop.Upgrades` table and this file knows the difference in exactly one place, the `AbilityTier_` loop: buying an ability is buying any other row. Upgrades cross to other services as attributes, the same channel Ghost and Freeze use: `BaseWalkSpeed` on the character, which is the baseline every `WalkSpeedResolver` factor multiplies, `MagnetBonus` on the player, which widens the pickup sweep, and one `AbilityTier_<Key>` per ability. The player attributes are stamped outside the character guard, because the ability bar draws itself from them and a profile landing a moment before the body would otherwise leave it blank. `applyStats` stamps `BaseWalkSpeed` and then calls `WalkSpeedResolver.apply` rather than writing the humanoid, because it re-runs on every purchase and a bare write would cancel whatever was live: buying Fast Feet mid-sprint used to slow you down. `TowerStart` touches are how furthest-section is credited. |
| PetService | (`EggPedestal`, to warn if none) | Owns the pet half of the profile: equip, XP, evolution, the follower rig, and the read-only projection every client draws from. Followers live in `workspace.LivePets`, anchored and eased by CFrame rather than pathfound: a pathfinding pet in a maze whose walls move is a pet stuck behind one. Two abilities are implemented. Glow is a `PointLight` on the rig, with the evolution multiplier scaling range and deliberately not brightness. Ward is the pet that is a defence: it scans for an enemy inside its radius, and when one arrives it publishes `WardRadius` on the rig for a few seconds and then recharges, which is the whole of its surface on this side. It never damages, stuns or moves anything, because there is no combat in this game; the enemy that reads the flag drops its target and walks home on its own legs. Triggered and cycled rather than always on, so it does not strictly dominate the Ghost orb and the Cloak ability, and it costs the only equip slot, so a safe maze is a dark one. Owns the `PetUpdate` and `PetIntent` remotes and the rate limit on the latter. |
...
| AbilityGui (client) | none | The ability bar, right column under the powerup chip: one button per owned ability, the shared charge under them, and the key. The bought things read down the right edge together, which is what left the sprint meter, free and permanent and belonging to no purchase, in the bottom right corner on its own. The bar is anchored to the right edge because its width is the number of abilities owned. Ownership, tier and selection are read straight off the replicated player attributes and never off the remote, so a client that joined late or reset its GUI is correct on its first frame; the remote supplies the charge and the events. Hidden entirely at zero owned, which is how a player who has not been to the stall is never told about a key they do not have. Hold `Q` to use, `1`-`9` or a tap to select, D-pad to cycle. |
...
- **Enemies cannot be damaged.** There is no combat, so a row's `health` and `Config.Enemies.HealthPerLevel` set a MaxHealth nothing reads and the `Died` path is reachable only in principle. It is wired correctly anyway (despawn, then respawn after `Config.Enemies.RespawnSeconds`), so adding a weapon is a weapon and not also a lifecycle. `EnemyController:takeDamage` is the entry point when one arrives. Two E4 types wait on it and are otherwise finished: the Splitter, whose split is on death, and the Warden's enrage, which is a health threshold nothing crosses.
