...
7. **Containment is a property of which function built the part.** `buildWalls` puts every wall it makes into the `MazeWall` collision group, through the single `wallPart` funnel, and no other function assigns a group. `buildFacade`, `buildSlab`, `buildStairs` and `buildRoof` leave theirs at Default. That is the whole of what stops the Wall Walker upgrade stranding anyone: a phasing player crosses any wall on a floor, lands in the apron ring between the maze edge and the facade with slab under their feet, and stops there, because the facade is not in the group. There is no runtime check to forget. Anything that adds a new wall-like part must go through `wallPart` or decide deliberately not to, and anything that adds a new *boundary* must not go in the group.
...
| EnemyService | `EnemySpawn` | Type from marker attribute, overridable per section in MazeConfig. Rigs from `ServerStorage/Enemies/<TypeName>`, procedural fallback. **A marker is permanent, its rig is not:** a rig is built when somebody comes within `Config.EnemySpawnRange` and destroyed past `Config.EnemyDespawnRange`, measured from the rig so a chase is never deleted mid-stride. Gating only the pathfinding, as the old version did, left all 180 markers per section holding a live Humanoid, and a server spending its frame on hundreds of idle state machines is why the few that mattered moved like they were underwater. Nothing blocks on `MoveToFinished`, whose eight second internal timeout was the rest of it; the think loop re-issues `MoveTo` each tick and decides arrival itself. Leash is measured from the marker, not the enemy, so an enemy owns a patch of maze and can be left behind; losing a target walks it back. Behaviour is per type off `profile.behavior` (Patrol/Stalk/Guard/Pack/Ambush/Charge), which is what the six types differ by now rather than a stud of walkSpeed. Enemies share a collision group that does not collide with itself, because three of them meeting in a corridor was most of "they just stand there". |
...
- **Enemy pathfinding scale.** Bounded by proximity rather than by section count: live rigs are only those near a player, so cost tracks players and not city size. The remaining flat cost is one sweep over every marker twice a second, about 3600 magnitude tests at five sections and four players. If section count ever grows far enough for that to matter, bucket the markers spatially in the scan loop; do not put the cost back into keeping rigs alive.
- **Enemies cannot be damaged.** There is no combat, so `Config.EnemyHealthBase`/`PerLevel` set a MaxHealth nothing reads and the `Died` path is reachable only in principle. It is wired correctly anyway (despawn, then respawn after `Config.EnemyRespawnSeconds`), so adding a weapon is a weapon and not also a lifecycle.
