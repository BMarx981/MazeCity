...
- Exit: `selene src/` clean, both modules present in a cold `rojo build`, and the require-time validation passing over the real catalogues, which is the exit criterion rather than a harness now that the check lives in the module itself. Plus one negative test: a deliberately bad key errors at require rather than loading quietly.
