...
- Depends on: **the login streak finding** (fragment 13 is caught at the claim in `DailyRewardService`, never polled, because the streak wraps to 1 past seven); and **the Gatekeeper finding** (14 has no source, so this unit must run correctly with that fragment simply unreachable, and with 15 to 17 sitting behind it, rather than treating a missing source as a bug). The **surviving decision is settled**: an encounter is survived when the rig gives up while the player is alive, which is a start moment paired with `onTargetLost` or a walk-away `stop` and cancelled by death. That pairing is this unit's to hold, it needs nothing new from the enemy system beyond the channel, and it is what makes 9 and 17 writable while 14 stays out of reach. The two fragment ids are in `Journal.lua` either way, unlocked by nothing until their source exists.
