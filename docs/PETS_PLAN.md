...
**In progress: units 1, 2 and 3 done plus unit 4's toast; unit 4's wall writings and unit 5 remain.** Defined by `docs/LORE.md` Section 10, which is the source of truth for every line of text this clutch ships; the audit above is where the game can be listened to. LORE.md's own exit criterion for the whole clutch is that a fresh save can unlock fragments 1 to 17 by playing, in order, with out-of-order accomplishments banked correctly.
...
- Depends on: units 1 through 3, and on **the roster conflicts**, because a completion meter over The Kept has to know how many Kept there are and today LORE.md and `EnemyDefinitions` disagree. It also inherits one thing unit 3 did not do: **`codex.pets` and `codex.kept` have no writer.** Unit 3 owns the journal chapter only, and the other three chapters are still the empty tables unit 2 made room for. `EnemyLore`'s encounter opens and closes are exactly the two moments `kept` was given a stage rather than a flag for (LORE.md line 81: silhouette and name on first encounter, lore line on surviving one), so that writer is a listener on the channel that already exists rather than anything new.
